...
void onLaunchMessage(json11::Json::object arguments)
{
    ...
    QemuContext ctx;
    QEMU_allocate_backed_drive(m3_string_format("%s/%s.img", std::get<1>(datastores.front()).c_str(), arn.c_str()), 32, "/home/gandalf/vms/ubuntu2004backingfile.img");
    if (-1 == QEMU_drive(ctx, m3_string_format("%s/%s.img", std::get<1>(datastores.front()).c_str(), arn.c_str()), 0))
    {
        return;
    }
